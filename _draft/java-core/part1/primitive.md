...
Приведенный ниже фрагмент кода демонстрирует как можно создать переменную каждого примитивного типа
...
Мы видим, что компилятор не может присвоить значения для данных типов, хотя они подходят по допустимому диапазону. Все дело в том, что Java автоматически воспринимает все числовые целочисленные константы в коде (`123`, `10000000`, `0` и т.п.) как тип `int`, а все вещественные (`3.14`, `0.25`, `0.0` и т.п.) - как тип `double`.
